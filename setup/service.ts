...
<dict>
    ...
    <string>${projectRoot}/logs/nonnaclaw.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/nonnaclaw.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.nonnaclaw');
  } catch {
...
/**
 * Kill any orphaned nonnaclaw node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned nonnaclaw processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/nonnaclaw.service';
    ...
    unitPath = path.join(unitDir, 'nonnaclaw.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=NonnaClaw Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/nonnaclaw.log
StandardError=append:${projectRoot}/logs/nonnaclaw.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned nonnaclaw processes to avoid WhatsApp conflict errors
  ...
  try {
    execSync(`${systemctlPrefix} enable nonnaclaw`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start nonnaclaw`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active nonnaclaw`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-nonnaclaw.sh');
  const pidFile = path.join(projectRoot, 'nonnaclaw.pid');

  const lines = [
    '#!/bin/bash',
    '# start-nonnaclaw.sh — Start NonnaClaw without systemd',
    ...
    '    echo "Stopping existing NonnaClaw (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting NonnaClaw..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/nonnaclaw.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/nonnaclaw.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "NonnaClaw started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/nonnaclaw.log"`,
  ];
